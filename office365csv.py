...
def csvInit():
    with open(r'D:\programming\python\csvChecker\office365\initial.csv') as initial, open(r'D:\programming\python\csvChecker\office365\manifest.csv') as manifest:
        ...
        for initialReader in initialTable:
             for manifestReader in manifestTable:  
                if initialReader['Email'].lower() == manifestReader['Email'].lower():
                    current_users.append(manifestReader['FirstName'] + "," + manifestReader['SurName'] + "," + manifestReader['Email'] + "," + initialReader['Location']) 
...
def csvCheckForTerm():
    ...
    for secondCheck in initialTable:
        for manifestReader in manifestTable:
            if secondCheck['Email'].lower() == manifestReader['Email'].lower():
                if secondCheck['Email'] not in terminated_users:
                    revised_users.append(secondCheck['FirstName'] + "," + secondCheck['SurName'] + "," + secondCheck['Email'] + "," + secondCheck['Location'] + "," + manifestReader['Licenses'])
                    break
    print("----------- TERMINATED ----------------------")
...
def csvCheckForNew():
    with open(r'D:\programming\python\csvChecker\office365\manifest.csv') as manifest:
        manifestTable = list(csv.DictReader(manifest))
        for manifestCheck in manifestTable: 
            if manifestCheck['Email'] not in current_users_email:
                if manifestCheck['Email'] not in terminated_users:
                    ...
                    new_users.append(manifestCheck['FirstName'] + "," + manifestCheck['SurName'] + "," + manifestCheck['Email'] + "," + "" + "," + manifestCheck['Licenses'])
    print("----------- NEW ----------------------")
    print(new_users_email)
    csvRewritten()

def csvRewritten():
    with open(r'D:\programming\python\csvChecker\office365\new_user.csv','w', newline="") as reconstruction: # takes the new users and prepare it to be sent to Google Spreadsheets
        ...
        rewriter.writerow(['FirstName,SurName,Email,Location,License'])
...
with open(r'D:\programming\python\csvChecker\office365\initial.csv',newline='') as i:
    j = csv.reader(i)
